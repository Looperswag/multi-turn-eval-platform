"""六大维度 v4 prompt 的 jinja2 模板字符串（A.3 改造）。

来源：原 prompts.py 内 build_dim{1-6}_prompt(*) 的 f-string 字面值。
转换规则：
- f-string 的 ``{var}`` → jinja2 ``{{ var }}``
- f-string 内 ``{{`` / ``}}`` 是字面花括号 → jinja2 里写为单 ``{`` / ``}``
- 原 f-string 中的 for 循环（history_text / turns_text）→ 改为 Python 端预拼接的字符串变量
  这样 jinja2 模板无需嵌套循环，且产物与 f-string 完全等价。

注意：jinja2 模板里出现的 JSON 大括号（如输出格式样例的 `{ "A": 1, ... }`）
直接写为单大括号即可——jinja2 仅把 `{{ }}` 视作变量插值，单个 `{` `}` 透传。

变量约定：
- dim1/3/4/5: history_text, current_user_query, current_rewritten_query
- dim2/6:     turns_text
"""
from __future__ import annotations


DIM1_TEMPLATE_V4 = """你是一个 query 改写质量评估专家。你需要判断"改写后的 query"是否忠实地反映了用户的真实意图。
...
DIM2_TEMPLATE_V4 = """你是一个多轮对话记忆保留能力评估专家。你需要判断系统的改写结果是否在多轮中正确保留了用户的关键约束。
...
DIM3_TEMPLATE_V4 = """你是一个对话边界处理能力评估专家。你需要判断系统当轮的改写 query 在面对边界输入时是否做出了合理处理。
...
DIM4_TEMPLATE_V4 = """你是一个指代消解能力评估专家。你需要判断系统是否正确将用户的指代词解析为具体商品。
...
DIM5_TEMPLATE_V4 = """你是一个重复搜索处理能力评估专家。你需要判断系统在用户发出"重复请求"时是否做出了正确处理。
...
DIM6_TEMPLATE_V4 = """你是一个用户纠错响应评估专家。你需要判断系统的后续改写是否吸收了用户的纠错信号。
...
ALL_V4_TEMPLATES: dict[str, str] = {
    "dim1": DIM1_TEMPLATE_V4,
    "dim2": DIM2_TEMPLATE_V4,
    "dim3": DIM3_TEMPLATE_V4,
    "dim4": DIM4_TEMPLATE_V4,
    "dim5": DIM5_TEMPLATE_V4,
    "dim6": DIM6_TEMPLATE_V4,
}
