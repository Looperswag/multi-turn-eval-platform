"""seed v4 prompt_template with jinja2 templates (A.3)

Revision ID: 0003
Revises: 0002
Create Date: 2026-05-14

A.3 改造：把六维 v4 JudgePromptVersion.prompt_template 从 placeholder marker
更新为 jinja2 模板字符串，供 PromptRenderer 渲染。

注：模板内联在本文件，避免 alembic import 链问题。
"""
...
DIM1_TEMPLATE = """你是一个 query 改写质量评估专家。你需要判断"改写后的 query"是否忠实地反映了用户的真实意图。
...
DIM2_TEMPLATE = """你是一个多轮对话记忆保留能力评估专家。你需要判断系统的改写结果是否在多轮中正确保留了用户的关键约束。
...
DIM3_TEMPLATE = """你是一个对话边界处理能力评估专家。你需要判断系统当轮的改写 query 在面对边界输入时是否做出了合理处理。
...
DIM4_TEMPLATE = """你是一个指代消解能力评估专家。你需要判断系统是否正确将用户的指代词解析为具体商品。
...
DIM5_TEMPLATE = """你是一个重复搜索处理能力评估专家。你需要判断系统在用户发出"重复请求"时是否做出了正确处理。
...
DIM6_TEMPLATE = """你是一个用户纠错响应评估专家。你需要判断系统的后续改写是否吸收了用户的纠错信号。
...
PLACEHOLDER_MARKER = (
    "v4 baseline — 当前 evaluator 仍从 app/services/eval_engine/prompts.py 读取硬编码 prompt；"
    "W2 阶段将切换为读取此字段。"
)
...
def upgrade() -> None:
    conn = op.get_bind()
    for dim_code, template in _TEMPLATES.items():
        conn.execute(
            sa.text(
                "UPDATE judge_prompt_version "
                "SET prompt_template = :tpl, "
                "    updated_at = now() "
                "WHERE dimension_code = :code AND version_tag = 'v4'"
            ),
            {"tpl": template, "code": dim_code},
        )
...
def downgrade() -> None:
    ...
    conn.execute(
        sa.text(
            "UPDATE judge_prompt_version "
            "SET prompt_template = :marker "
            "WHERE version_tag = 'v4'"
        ),
